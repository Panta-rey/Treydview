...
**Stand: 17. August 2026 · Build m60**
...
## Build m60 — Mobile-Parität (17. August 2026)

Desktop-Features auf Mobile nachgezogen. Alles hinter
`@media(max-width:720px),(pointer:coarse)` bzw. `tvIsMobile()` — Desktop unberührt.

**Niedrig-Gruppe:**
- **Sync-Button** in die Mobile-Verteilung aufgenommen (tbRow1 nach faqBtn).
- **Icon-Grössen** der tbRow1-Knöpfe auf 34x30 (kompakt wie Vergleich-Icon).
- **Doppeltipp-Zoom** via `body{touch-action:manipulation}` unterbunden.
- **Pane-Icons** (4a): Collapse 21px, Zahnrad 15px.

**Hoch-Gruppe:**
- **2 (Zeichentools)** — Mobile-Draw-Sheet aus `DRAW_CATEGORIES` abgeleitet
  (eine Quelle) statt eigener Liste: gleiche Kategorien/Reihenfolge/Tools,
  inkl. Parallele Linien und Preis-/Zeitspanne, ohne Duplikate. Touch-Darstellung.
- **4b (Umsortieren)** — `startPaneDrag` + ph-drag um touchstart/touchmove/
  touchend erweitert (Koord. aus touches/changedTouches, preventDefault gegen
  Mitscrollen).
- **4b/4c (Skalieren + Höhe ziehen)** — Ursache: der Pane-Kopf (DOM-Overlay,
  volle Breite) verdeckte auf Touch KLCs Separator und den oberen Achsenbereich
  und fing die Gesten ab. Fix: `.pane-header{pointer-events:none}` +
  `.pane-header .ph-btn{pointer-events:auto}` (nur Mobil) — Icons interaktiv,
  Rest lässt Touches an den Chart/Separator durch. **Am Gerät zu verifizieren.**
- **Vergleich-%-Skala** — auf Mobile rechtsbündig (contentW=w statt
  Zeichenbereich, da Preisskala im Vergleich ohnehin ausgeblendet).
- **Preisskala-Format** — auf Mobile `setPriceVolumePrecision` nach Preisniveau
  (>=1000 -> 0, >=1 -> 2, sonst 4 Nachkommastellen).

**Lektion m60:** DOM-Overlays über KLC-Panes (Kopfzeilen) müssen auf Touch
`pointer-events` gezielt freigeben, sonst schlucken sie Separator-/Achsen-Gesten.
Touch-Handler brauchen eigene Koordinaten-Extraktion (touches/changedTouches).
...
| Datei | MD5 | Zeilen |
|---|---|---|
| index.html | aa982f8161015683eae8afd858dc3afc | — |
| style.css | 0839295290c198692f0b405ff3e881a9 | — |
| app.js | 53e6e3b177542ce3478f66c28d142cc5 | — |
...
> **m60 geändert:** app.js, style.css, index.html (nur ?v-Bump).
> overlays.js, settings.js, klinecharts.min.js, config.js, data.js,
> worker-komplett.js unverändert — kein Bundle-/Worker-/settings-Deploy nötig.
...
## Build m65 — Änderungen (20. August 2026)

Feinschliff-Runde nach m64. Deploy-Hinweis: Repo nutzt js/- und css/-Unterordner;
gelieferte Dateien gehören nach js/app.js, css/style.css, js/indicators.js,
index.html (Wurzel). index.html trägt bereits die js/css-Pfade + fibGoldenPocket.

### Desktop & Mobile
- **P1 — Collapse-Icon gleiche Form:** ph-collapse nutzt eine Glyphe (▾); eingeklappt
  per Klasse .collapsed + CSS transform:rotate(180deg). Garantiert identische Form.
- **P2 — Werteskala Präzision:**
  - Preis-Achse (setPriceVolumePrecision): Schwelle 1000→100, jetzt Desktop UND Mobil
    (tvIsMobile-Gate entfernt). >100 → 0 Dezimalstellen.
  - Preis-Tag (formatTagValue price): Schwelle 1000→100.
  - Indikator-Tag (formatTagValue !price): >10 → 0 Dezimalstellen (Math.round +
    Tausender-Trenner), sonst toFixed(2).
  - Indikator-Achse (Variante a, feste Präzision): MYRSI, STOCHRSI, GLOBALM2 → 0.
    MACD/ATR bleiben 2 (asset-abhängig, bei Sub-1-Assets braucht es Dezimalstellen).
    Preis-Overlays (EMA/SMA/BOLL/Hull/VWAP/GC/BMSB/MNOODLE) bleiben 2 (nutzen die
    dynamische Preis-Achse + dynamische Tags).

### Mobil
- **HF1 — Sync-Abstand:** m64-margin griff sichtbar nicht (vermutlich Copy-Lücke im
  Deploy). Jetzt über den zuverlässigen .tb-row::after-Flex-Spacer (8→14px) + der
  bestehende #syncBtn margin-right:8px. Am Gerät zu verifizieren.
- **HF2/QF2 — FAQ-Text zu gross:** Ursache mobiles Font-Boosting (Brave/iOS bläht
  lange Textblöcke auf). Fix: html,body { -webkit-text-size-adjust:100%;
  text-size-adjust:100% } — Text bleibt bei den CSS-12px. Desktop wirkungslos.
- **QF1 — Watchlist heller:** .watchlist:not(.hidden){ background:var(--bg-hover) }
  (eine Stufe heller als FAQ/--bg-raised). Layout-Modal bleibt --bg-raised.
- **QF3 — Dropouts zeigten nur den Griff:** .dd-anchored bekam z-index:644 zurück
  (war beim Sheet-Ausschluss verloren → Panels auf Basis-200, im Querformat verdeckt,
  Griff bei 645 blieb sichtbar).
- **QF4 — Zeichendropout Leerraum:** padding-left 16→12px (knapp hinter den Griff bei
  5–10px, kein Overlap).
- **QF5 — Long/Short-Fenster:** posToolTopBtn-Handler schliesst offenes lsChoice bei
  erneutem Klick (statt neu zu öffnen).

### Robustheit
- openFibMenu/_fibLiveApply lesen fibGoldenPocket jetzt null-tolerant (?.) — falls die
  Checkbox in einer alten index.html fehlt, crasht das Fib-Menü nicht.

### Lektionen
- Mobiles „Font Boosting" (Chrome/Brave/iOS) vergrössert lange Textblöcke automatisch,
  obwohl das CSS eine kleine Grösse vorgibt — mit text-size-adjust:100% abschaltbar.
- Beim Ausschluss aus einer Regel per :not() gehen auch deren übrige Properties
  (hier z-index) verloren — separat wiederherstellen.
- KLineCharts formatiert Achsen-Ticks per fester Indikator-precision, nicht per Wert —
  „>10 keine Dezimalstellen" ist dort nur über feste per-Indikator-Präzision lösbar
  (bounded/grosse → 0, asset-abhängige → 2).
